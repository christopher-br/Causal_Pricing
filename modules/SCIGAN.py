...
from utils.eval_utils import compute_val_SCIGAN
...
class SCIGAN_Model:
    def __init__(self, params):
        ...
        self.num_iter_generator = params['num_iter_generator']
        ...
        self.X = tf.placeholder(
            tf.float32, shape=[None, self.num_features], name='input_features')
        # Treatment (T) - one-hot encoding for the treatment
        self.T = tf.placeholder(
            tf.float32, shape=[None, self.num_treatments], name='input_treatment')
        # Dosage (D)
        self.D = tf.placeholder(
            tf.float32, shape=[None, 1], name='input_dosage')
        # Dosage samples (D)
        self.Treatment_Dosage_Samples = tf.placeholder(tf.float32,
                                                       shape=[
                                                           None, self.num_treatments, self.num_dosage_samples],
        ...
        # Random Noise (G)
        self.Z_G = tf.placeholder(
            tf.float32, shape=[None, self.size_z], name='input_noise')

    def generator(self, x, y, t, d, z, treatment_dosage_samples):
        with tf.variable_scope('generator', reuse=tf.AUTO_REUSE):
            inputs = tf.concat(axis=1, values=[x, y, t, d, z])
            G_shared = tf.layers.dense(
                inputs, self.h_dim, activation=tf.nn.elu, name='shared')
            ...
            for treatment in range(self.num_treatments):
                ...
                treatment_dosages = tf.reshape(
                    treatment_dosages, shape=(-1, 1))
                G_shared_expand = tf.reshape(tf.tile(G_shared, multiples=[1, self.num_dosage_samples]),
                                             shape=(-1, self.h_dim))
                input_counterfactual_dosage = tf.concat(
                    axis=1, values=[G_shared_expand, treatment_dosages])
                ...
                # The output layer
                treatment_dosage_output = tf.layers.dense(treatment_layer_2, 1, activation=tf.nn.sigmoid,
                                                          name='treatment_output_%s' % str(
                                                              treatment),
                                                          reuse=tf.AUTO_REUSE)

                dosage_counterfactuals = tf.reshape(
                    treatment_dosage_output, shape=(-1, self.num_dosage_samples))

                G_treatment_dosage_outcomes[treatment] = dosage_counterfactuals

            G_logits = tf.concat(
                list(G_treatment_dosage_outcomes.values()), axis=1)
            G_logits = tf.reshape(
                G_logits, shape=(-1, self.num_treatments, self.num_dosage_samples))
    ...
    def dosage_discriminator(self, x, y, treatment_dosage_samples, treatment_dosage_mask,
        ...
        with tf.variable_scope('dosage_discriminator', reuse=tf.AUTO_REUSE):
            observation_features_representation = tf.expand_dims(tf.layers.dense(x, self.h_dim, activation=tf.nn.elu),
                                                                 axis=1)
            ...
            for treatment in range(self.num_treatments):
                ...
                dosage_potential_outcomes = tf.expand_dims(
                    G_treatment_dosage_outcomes[treatment], axis=-1)

                inputs = tf.concat(
                    axis=-1, values=[dosage_samples, dosage_potential_outcomes])
                D_h1 = tf.nn.elu(equivariant_layer(inputs, self.h_inv_eqv_dim, layer_id=1,
                                                   treatment_id=treatment) + observation_features_representation)
                D_h2 = tf.nn.elu(equivariant_layer(
                    D_h1, self.h_inv_eqv_dim, layer_id=2, treatment_id=treatment))
                D_logits_treatment = tf.layers.dense(D_h2, 1, activation=None,
                                                     name='treatment_output_%s' % str(treatment))

                D_dosage_outcomes[treatment] = tf.squeeze(
                    D_logits_treatment, axis=-1)

            D_dosage_logits = tf.concat(
                list(D_dosage_outcomes.values()), axis=-1)
            D_dosage_logits = tf.reshape(
                D_dosage_logits, shape=(-1, self.num_treatments, self.num_dosage_samples))
    ...
    def treatment_discriminator(self, x, y, treatment_dosage_samples, treatment_dosage_mask,
        ...
        with tf.variable_scope('treatment_discriminator', reuse=tf.AUTO_REUSE):
            observation_features_representation = tf.layers.dense(
                x, self.h_dim, activation=tf.nn.elu)
            ...
            for treatment in range(self.num_treatments):
                ...
                inputs = tf.concat(
                    axis=-1, values=[dosage_samples, dosage_potential_outcomes])
                D_treatment_rep = invariant_layer(
                    x=inputs, h_dim=self.h_inv_eqv_dim, treatment_id=treatment)

                D_treatment_outcomes[treatment] = D_treatment_rep

            D_treatment_representations = tf.concat(
                list(D_treatment_outcomes.values()), axis=-1)
            D_shared_representation = tf.concat(
                [D_treatment_representations, observation_features_representation], axis=-1)
        ...
        with tf.variable_scope('inference', reuse=tf.AUTO_REUSE):
            ...
            for treatment in range(self.num_treatments):
                ...
                for index in range(self.num_dosage_samples):
                    dosage_sample = tf.expand_dims(
                        treatment_dosages[:, index], axis=-1)
                    input_counterfactual_dosage = tf.concat(
                        axis=1, values=[I_shared, dosage_sample])

                    treatment_layer_1 = tf.layers.dense(input_counterfactual_dosage, self.h_dim, activation=tf.nn.elu,
                                                        name='treatment_layer_1_%s' % str(
                                                            treatment),
                                                        reuse=tf.AUTO_REUSE)

                    treatment_layer_2 = tf.layers.dense(treatment_layer_1, self.h_dim, activation=tf.nn.elu,
                                                        name='treatment_layer_2_%s' % str(
                                                            treatment),
                                                        reuse=tf.AUTO_REUSE)
                ...
                I_treatment_dosage_outcomes[treatment] = tf.concat(
                    list(dosage_counterfactuals.values()), axis=-1)

            I_logits = tf.concat(
                list(I_treatment_dosage_outcomes.values()), axis=1)
            I_logits = tf.reshape(
                I_logits, shape=(-1, self.num_treatments, self.num_dosage_samples))
    ...
    def train(self, Train_X, Train_T, Train_D, Train_Y, verbose=False):
        ...
        # 4. Inference network
        I_logits, I_treatment_dosage_outcomes = self.inference(
            self.X, self.Treatment_Dosage_Samples)
        ...
        idx = tf.stack(
            [tf.range(num_examples), factual_treatment_idx], axis=-1)
        ...
        G_logit_factual = tf.expand_dims(tf.reduce_sum(
            self.Treatment_Dosage_Mask * G_logits, axis=[1, 2]), axis=-1)
        G_loss_R = tf.reduce_mean((self.Y - G_logit_factual) ** 2)
        G_loss = self.alpha * tf.sqrt(G_loss_R) + G_loss_GAN

        # 4. Inference loss
        I_logit_factual = tf.expand_dims(tf.reduce_sum(
            self.Treatment_Dosage_Mask * I_logits, axis=[1, 2]), axis=-1)
        ...
        # Use I_loss2 for hyperparameter tuning

        theta_G = tf.trainable_variables(scope='generator')
        theta_D_dosage = tf.trainable_variables(scope='dosage_discriminator')
        theta_D_treatment = tf.trainable_variables(
            scope='treatment_discriminator')
        theta_I = tf.trainable_variables(scope='inference')

        G_solver = tf.train.AdamOptimizer(
            learning_rate=0.001).minimize(G_loss, var_list=theta_G)
        D_dosage_solver = tf.train.AdamOptimizer(learning_rate=0.001).minimize(
            D_dosage_loss, var_list=theta_D_dosage)
        D_treatment_solver = tf.train.AdamOptimizer(learning_rate=0.001).minimize(D_treatment_loss,
                                                                                  var_list=theta_D_treatment)
        I_solver = tf.train.AdamOptimizer(
            learning_rate=0.001).minimize(I_loss, var_list=theta_I)
        ...
        # Iterations
        for it in tqdm(range(self.num_iter_generator), leave=False, desc="Training SCIGAN GAN", ncols=125):
            for kk in range(2):
                ...
                factual_dosage_position = np.random.randint(
                    self.num_dosage_samples, size=[self.batch_size])
                treatment_dosage_samples[range(
                    self.batch_size), T_mb, factual_dosage_position] = D_mb

                treatment_dosage_mask = np.zeros(shape=[self.batch_size, self.num_treatments,
                                                        self.num_dosage_samples])
                treatment_dosage_mask[range(
                    self.batch_size), T_mb, factual_dosage_position] = 1
            ...
            if it % 1000 == 0 and verbose:
                ...
                print('D_loss_treatments: {:.4}'.format(
                    (D_treatment_loss_curr)))
                print('D_loss_dosages: {:.4}'.format((D_dosage_loss_curr)))
                print('G_loss: {:.4}'.format((G_loss_curr)))
                print()

        # Train Inference Network
        for it in tqdm(range(self.num_iter_inference), leave=False, desc="Training SCIGAN MLP", ncols=125):
            idx_mb = sample_X(Train_X, self.batch_size)
            X_mb = Train_X[idx_mb, :]
            T_mb = np.reshape(Train_T[idx_mb], [self.batch_size, ])
            D_mb = np.reshape(Train_D[idx_mb], [self.batch_size, ])
    ...
    def tune(self, Train_X, Train_T, Train_D, Train_Y, dataset_val, batch_sizes, h_dims, nums_dosage_samples):
        ...
        params_help['num_iter_generator'] = self.num_iter_generator
        params_help['num_iter_inference'] = self.num_iter_inference
        params_help['export_model_dir'] = 'outputs/saved_models/SCIGAN_train'

        # For progress tracker
        tuning_loops = len(batch_sizes) * len(h_dims) * \
            len(nums_dosage_samples)

        # Get progress bar
        pbar = tqdm(desc="Tuning SCIGAN", leave=False,
                    ncols=125, total=tuning_loops)

        # Iterate over training parameters
        for batch_size in batch_sizes:
            params_help['batch_size'] = batch_size

            for num_dosage_samples in nums_dosage_samples:
                params_help['num_dosage_samples'] = num_dosage_samples

                for h_dim in h_dims:
                    params_help['h_dim'] = h_dim
                    params_help['h_inv_eqv_dim'] = h_dim

                    mod = SCIGAN_Model(params_help)
                    ...
                    err = compute_val_SCIGAN(dataset_val=dataset_val,
                    ...
                    if err < best_val_error:
                        ...
                        with tf.Session(graph=tf.Graph()) as sess:
                            tf.saved_model.loader.load(
                                sess, ["serve"], params_help['export_model_dir'])
